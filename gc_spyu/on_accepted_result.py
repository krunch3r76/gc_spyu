...
NodeInfoNT = namedtuple(
    "NodeInfoNT", ["nodeInfoId", "providerId", "modelname", "unixtime", "nodename"]
)  # unixtime is Decimal
...
def _add_nodeInfo(myModel, providerId, intel_d, attached_prefix):
    ...
    inserted_nodeInfoId = myModel.execute_and_id(
        ...
        + " VALUES(?, ?, ?, ?)",
        [providerId, intel_d["model"], Decimal(intel_d["unixtime"]), intel_d["name"]],
    )[0]
...
def _add_provider(myModel, addr, attached_prefix=""):
    ...
    providerId = myModel.execute_and_id(
        "INSERT INTO " + f"{attached_prefix}provider(addr)" + "VALUES (?)", [addr]
    )[0]
    return providerId
...
def _lookup_provider_id(myModel, addr, attached_prefix=""):
    if attached_prefix != "":
        assert "." in attached_prefix, "malformed attached db prefix"
    recs = myModel.execute(
        "SELECT providerId, addr FROM "
...
def update_main_db(myModel, intel_d):
    ...
    def __update_nodeInfo_time(nodeInfoId, unixtime_str):
        myModel.execute(
            "UPDATE nodeInfo SET unixtime = (?) WHERE" + f" nodeInfoId = {nodeInfoId}",
            [Decimal(unixtime_str)],
        )
    ...
    def __lookup_last_nodeinfo_record(providerId) -> NodeInfoNT:
        ...
        nodeInfoRows = myModel.execute(
            "SELECT nodeInfoId, providerId"
            + ", modelname"
        ...
        debug.dlog(
            f"timestamp for {rv_nodeInfoRecord.nodename}:"
    ...
    if providerId == None:
        ...
        if nodeInfoRec != None:
            if not (
                intel_d["name"] == nodeInfoRec.nodename
                and intel_d["model"] == nodeInfoRec.modelname
            ):
                _add_nodeInfo(myModel, providerId, intel_d, prefixAttached)
            else:
                __update_nodeInfo_time(nodeInfoRec.nodeInfoId, intel_d["unixtime"])
...
def update_extra_db(myModel, intel_d, offer: str, agr_id):
    ...
    def __add_offer(nodeInfoId, offer):
        myModel.execute(
            f"INSERT INTO {prefixAttached}offer(nodeInfoId," + "data) VALUES (?, ?)",
            [nodeInfoId, offer],
        )
    ...
    def __add_agreement(nodeInfoId, agr_id):
        myModel.execute(
            f"INSERT INTO {prefixAttached}agreement(nodeInfoId," + "id) VALUES (?, ?)",
            [nodeInfoId, agr_id],
        )
...
def on_accepted_result(myModel, mySummaryLogger):
    ...
    async def on_accepted_result_closure(result):
        ...
        extraNodeInfoId = update_extra_db(
            myModel, gatheredIntel, json.dumps(result["offer"]), result["agr_id"]
        )
        myModel.commit()
